...
RSpec.describe 'Index of all Warehouses page', type: :system do
  it 'has a table of warehouses', :js do
    ...
    warehouse = create(:warehouse, street: '123 test', city: 'test city', province: 'test province')

    visit '/warehouses'

    expect(page).to have_a_warehouses_table
    ...
    expect(page).to have_column_for('street', value: '123 test', record: warehouse)
    expect(page).to have_column_for('city', value: 'test city', record: warehouse)
    expect(page).to have_column_for('province', value: 'test province', record: warehouse)
  ...
  it 'allows to delete a warehouse', :js do
    ...
    # expect(page).not_to have_column_for('sku', value: 'SKU-001', record: warehouse)
    expect(page).to have_success_delete_message(warehouse.id)
  end

  private

  def have_a_warehouses_table
    have_css('table#warehouses-table')
  end
